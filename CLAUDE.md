...
`appveyor.yml` builds `src/Serialize.Linq.sln` in Release on Windows and publishes the resulting `.nupkg` (+ symbols) to NuGet on pushes to `main`. The package version is driven by the AppVeyor build version and the `<Version>` in the csproj.
